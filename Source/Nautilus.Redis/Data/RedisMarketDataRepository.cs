...
namespace Nautilus.Redis.Data
{
  ...
  public sealed class RedisMarketDataRepository : DataBusConnected, ITickRepository, IBarRepository, IRepository
  {
    ...
    public RedisMarketDataRepository(
      ...
      IReadOnlyDictionary<BarStructure, int> retentionTimeBarsDays) : base(container, dataBusAdapter)
    {
    ...
    {
      return 
        this.KeyExists(KeyProvider.GetPricesKey(symbol, PriceType.Bid)) &&
    ...
    {
      return this.KeyExists(key) ? this.redisDatabase.TimeSeriesInfo(key).TotalSamples : 0;
    }
      ...
      long? limit)
    {
      ...
      switch (priceType)
      {
        ...
        default: throw ExceptionFactory.InvalidSwitchArgument(priceType, nameof(priceType));
      }
    }
